  ...
  <div>
    ...
    <div class="jeecg-basic-table-form-container">
      <a-form @keyup.enter.native="searchQuery" :model="queryParam" :label-col="labelCol" :wrapper-col="wrapperCol">
        <a-row :gutter="16">
          <a-col :lg="8">
            <a-form-item label="账号等级">
              <j-dict-select-tag placeholder="请选择账号等级" v-model:value="queryParam.levelId" dictCode="account_level"/>
            </a-form-item>
          </a-col>
          <a-col :lg="8">
          ...
          <a-col :lg="8">
            <a-form-item label="账号">
              <a-input placeholder="请输入账号" v-model:value="queryParam.account" ></a-input>
            </a-form-item>
          </a-col>
          <a-col :lg="8">
              <a-form-item label="账号昵称">
                <a-input placeholder="请输入账号昵称" v-model:value="queryParam.nickName"></a-input>
              </a-form-item>
          </a-col>
          <a-col :lg="8">
            <a-form-item label="游戏名">
              <a-select v-model:value="gameId"   placeholder="请选择游戏名" allowClear @change="handleChange">
                <a-select-option
          ...
          <a-col :lg="8">
            <a-form-item label="子游戏名">
              <a-select v-model:value="subGameId" allowClear placeholder="请选择子游戏名" v-if="gameId !=='' && gameId !==undefined">
                <a-select-option
    ...
    <!-- 表单区域 -->
    <OpPutAccountModal ref="registerModal" @success="handleSuccess"></OpPutAccountModal>
  </div>
</template>
...
<script lang="ts" name="opputaccount-opPutAccount" setup>
  ...
  import { columns, } from './OpPutAccount.data';
  import { list, deleteOne, batchDelete, getImportUrl, getExportUrl, auth, syncAccount, faList } from './OpPutAccount.api';
  import { downloadFile } from '/@/utils/common/renderUtils';
  import OpPutAccountModal from './components/OpPutAccountModal.vue';
  ...
  function toSyncAccount(record){
    if(record.levelId != 1){
      message.warning('只有一级账号可以进行进行同步');
      return;
    }
    syncAccount({accountId : record.id}); 
  }
