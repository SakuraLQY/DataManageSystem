<template>
  ...
  <SummaryCard @sonDataName = "lineCard"/>
  <CtDailySummary :num ="tempNum"/>
</template>
...
<script lang="ts" name="count-ctDailySummary" setup>
  ...
  import { Item, columns } from './CtDailySummary.data';
  ...
  import CtDailySummary from './components/CtDailySummary.vue';
  ...
  const tempNum = ref(0);
	//父组件传到子组件
  provide('tempN',tempNum);
//将子组件的值传到这里，通过@sonData
  const lineCard = (e)=>{
    //将子组件传来的值拿到
    tempNum.value = e;
  }
  ...
  //     const todayData = await queryList(queryParam.value);
