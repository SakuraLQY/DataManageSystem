<template>
  <div>
        ...
        <a-row :gutter="24">
          ...
          <template v-if="toggleSearchStatus">
            ...
            <a-form-item label="回调状态">
                <j-search-select placeholder="请选择回调状态" v-model:value="queryParam.callbackState"  dict="callback_state" allowClear />
            </a-form-item>
          </a-col>
          <a-col :lg="8">
    ...
    <!-- 表单区域 -->
    <CtCallbackModal ref="registerModal" @success="handleSuccess"></CtCallbackModal>
  </div>
</template>

<script lang="ts" name="callback-ctCallback" setup>
  import { ref, reactive } from 'vue';
  import { BasicTable, useTable, TableAction } from '/@/components/Table';
  import { useListPage } from '/@/hooks/system/useListPage';
  import { columns } from './CtCallback.data';
  import { list, deleteOne, batchDelete, getImportUrl, getExportUrl, callbackOperation } from './CtCallback.api';
  import { downloadFile } from '/@/utils/common/renderUtils';
  import CtCallbackModal from './components/CtCallbackModal.vue'
   ...
   * 操作 
   */
  function handleOperation(record: Recordable, operationType) {
    callbackOperation({ id: record.id, operationType:operationType  }, handleSuccess);
  ...
  function getTableAction(record) {
    return [
      {
        label: '激活',
        onClick: handleOperation.bind(null, record,1),
      },
      {
        label: '注册',
        onClick: handleOperation.bind(null, record,2),
      },
      {
        label: '付费',
        onClick: handleOperation.bind(null, record,3),
      },
    ];
  }
