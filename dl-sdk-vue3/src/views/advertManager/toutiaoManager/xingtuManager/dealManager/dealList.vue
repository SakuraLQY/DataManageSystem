<template>
  <div>
        ...
        <a-row :gutter="24">
          ...
          <a-col :lg="8">
            <a-form-item label="渠道子账号ID">
              <j-search-select
                ...
                dict="open_gateway.op_channel_sub_account where channel_id = 5,sub_account_name,id"
                :disabled="disabled"
          ...
          <a-col :lg="8">
            <a-form-item label="主播">
              <j-search-select placeholder="请选择主播" v-model:value="queryParam.anchorId" dict="open_gateway.op_anchor_plan,plan_name,id" allowClear/>
            </a-form-item>
          </a-col>
          <a-col :lg="8">
    ...
    <BasicTable @register="registerTable" :rowSelection="rowSelection">
      ...
      <!-- 主播状态 -->
      <template #anchorPlanBindDeal_data="{ record }">
        <a-tag color="error" v-if="!record.anchorPlanId" >未绑定</a-tag>
        <a-tag color="success" v-else-if="record.id == record.anchorPlanBindDeal" >当前绑定</a-tag>
        <a-button type="primary" shape="round" v-else @click="bingDeal(record.id, record.anchorPlanId)">设为绑定</a-button>
      </template>
      <!-- 安装包下载链接 -->
      <template #pkgUrl_data="{ record }">
        <a-radio-group size="small">
          <a-tooltip>
            ...
            <a-radio-button size="small">常规</a-radio-button>
          </a-tooltip>
          <a-tooltip>
            <template #title>{{ record.androidUrl }}</template>
            <a-radio-button size="small">安卓</a-radio-button>
          </a-tooltip>
          <a-tooltip>
            <template #title>{{ record.iosUrl }}</template>
            <a-radio-button size="small">IOS</a-radio-button>
          </a-tooltip>
        </a-radio-group>
      </template>
      <!-- 打包状态 -->
      <template #packState_data="{ record }">
          ...
          <a-tag color="error" v-if="record.packState == 4">打包失败</a-tag>
      </template>
      <!-- 站点状态 -->
      <template #siteStatus_data="{ record }">
        <!-- 文字提示 -->
        <a-tooltip v-if="record.jrttSiteId != null">
          <template #title>https://www.chengzijianzhan.com/tetris/page/{{ record.jrttSiteId }}</template>
    ...
    <!-- 表单区域 -->
    <DealModal ref="registerModal" @success="handleSuccess"></DealModal>
    <JrttSiteModal ref="siteRegisterModal" :successCallback="reload" :siteSave="siteSave"></JrttSiteModal>
  </div>
</template>
...
<script lang="ts" name="vendor-jrttDeal" setup>
  ...
  import { columns, subGameTypeOptions, packTypeOptions, jrttPkg } from './deal.data';
  import { list, deleteOne, batchDelete, getImportUrl, getExportUrl, bind, siteSave } from './deal.api';
  ...
  import JrttSiteModal from '/@/views/advertManager/toutiaoManager/jrttdeal/components/JrttSiteModal.vue';
   ...
   * 主播绑定广告
   */
  function bingDeal(dealId, anchorId) {
    bind({dealId:dealId, anchorId:anchorId}, handleSuccess);
  }
