...
<script lang="ts" setup>
  ...
  import { queryDataPermissionTreeList, queryDataPermission, saveDataPermission } from './user.api';
  ...
  const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
    ...
    //初始化数据
    const dataResult = await queryDataPermissionTreeList();
    console.log(1)
    if(dataResult.treeList && dataResult.treeList.length > 1){
      console.log(222)
      dataResult.treeList[0]['disableCheckbox'] = true;
      dataResult.treeList[1]['disableCheckbox'] = true;
    }
    treeData.value = dataResult.treeList;
    allTreeKeys.value = dataResult.ids;
    //初始化角色菜单数据
    const permResult = await queryDataPermission({userId: data.userId });
  ...
  function onTreeNodeSelect(key) {
    ...
    openDataRuleDrawer(true, { functionId: unref(selectedKeys)[0] });
  }
  /**
   * 数据重置
   */
  ...
  async function handleSubmit(exit) {
    ...
    loading.value = true;
    await saveDataPermission(params);
    loading.value = false;
    exit && closeDrawer();
  }
</script>
